...
<div id="post-<?php the_ID(); ?>" <?php post_class('blog-item'); ?>>
    ...
    if ( is_single() ) :
        ...
        if ( $coffee_tea_single_post_feature_image == '1' ) :
            if ( has_post_thumbnail() ) :
                echo '<div class="blog-thumb">' . get_the_post_thumbnail() . '</div>';
            else :
                // Using a default image for posts without a featured image
                $coffee_tea_default_image_url = esc_url( get_template_directory_uri() . '/assets/images/slider.png' );
                echo '<div class="blog-thumb"><img src="' . $coffee_tea_default_image_url . '" alt="' . esc_attr__( 'Default Image', 'coffee-tea' ) . '"></div>';
            endif;
        endif;
    else :
        // Archive post settings
        if ( $coffee_tea_post_feature_image == '1' ) :
            if ( has_post_thumbnail() ) :
                echo '<div class="blog-thumb">' . get_the_post_thumbnail() . '</div>';
            else :
                // Using a default image for posts without a featured image
